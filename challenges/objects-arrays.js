...
const universities = graduates.map(current => current.university);
...
const contactInfo = graduates.map(current => `${current.first_name} ${current.email}`);
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter(current => current.university.includes('Uni'));
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach((current, index) => animalNames[index] = `Name: ${current.animal_name}, Scientific: ${current.scientific_name}.`);
...
const lowerCase = zooAnimals.map(current => current.animal_name.toLowerCase());
...
*/
const lowerPopulation = zooAnimals.filter(current => current.population < 5);
